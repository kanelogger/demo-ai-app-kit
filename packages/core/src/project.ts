...
function skipFilter(src: string): boolean {
  const basename = src.split("/").pop()!;
  if (basename === "node_modules" || basename === "dist") return false;
  if (basename.startsWith(".") && basename !== ".gitignore" && basename !== ".agents") return false;
  return true;
}
...
async function renderProjectFile(filePath: string, vars: Record<string, string>): Promise<void> {

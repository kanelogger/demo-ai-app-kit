...
面向 AI Coding 场景的 PC 后台项目生成工具包，用于从一个简单业务需求生成可运行、可维护、可验收的后台管理/工作台应用。

## 项目定位

`demo-ai-app-kit` 不是固定业务系统，也不是面向单次活动的临时脚手架。它是一条 PC 后台项目生产线：给 Agent 一个业务需求，Agent 基于项目内模板、提示词、技能和验收脚本，产出可本地运行、可维护、可验收的后台管理/工作台应用。
...
-> 一问一答澄清关键问题
-> 输出需求文档
-> 输出 3 个技术方案 + 推荐方案 + 选择记录
-> 冻结选定方案并输出技术方案 + 工作流集成 + 测试用例
-> 按方案从 shell 实现代码
-> 联调测试 + check-demo
-> 本地验收与质量审查
```

项目可以服务演示、评测和交付验收场景，但核心目标是生成 PC 后台应用的质量：需求清楚、方案选择可追溯、接口稳定、代码可读、能本地运行、有 mock fallback、有测试/验收、有轻量审查、有可验证的本地运行与质量审查。
...
- `templates/docs/solution-options.md`：要求在技术方案和实现前输出 3 个可选方案、推荐方案和用户/Agent 选择记录。
- `templates/docs/skill-trace.md`：记录实际使用的 skills、产物、跳过原因和命令证据。
- `bin/check-docs`：实现前文档包质量检查，覆盖需求、技术方案、workflow 文档和测试用例，不启动应用。
- `bin/check-demo`：两阶段 demo readiness 检查；kit 仓库检查生成器、模板和公共说明，generated app 检查 Agent 是否已填完标准产物。
- `docs/sample-doc-validation/`：三组样例业务文档包，用于验证模板能覆盖社区报修、合同风险审核和设备巡检派单。
- `docs/checklists/demo-readiness.md`：本地 demo 验收清单。
- `docs/checklists/skill-pruning.md`：项目内技能筛选和 workflow。
- `templates/docs/`：生成项目的标准文档骨架（`requirements.md`、`solution-options.md`、`tech-plan.md`、`workflow-integration.md`、`test-cases.md`、`skill-trace.md`、`test-report.md`）。
...
- 增加 `bin/demo-ai-app`：支持 `demo-ai-app <project-name>` 生成待 Agent 填充的项目工作区，自动复制 `AGENTS.md`、入口 prompt、核心技能、`bin/check-demo`、`bin/check-docs`、模板代码和标准产物骨架。
...
5. 优先复用模板，不从空白项目开始。
6. 行为逻辑、adapter 和 mock fixture 默认有测试；PC 后台应用完成后默认做浏览器验证。
...
- `skills/tech-plan-generator/`：在方案选择后生成页面、数据模型、接口、工作流契约、SDD-lite 和验证计划。
...
- `skills/webapp-testing/`：用 Playwright 验证本地 PC 后台应用。
- `skills/debugging-and-error-recovery/`：构建、测试或运行失败时定位根因。
- `skills/code-review-and-quality/`：提交或交付前做轻量质量审查。
- `skills/release-readiness/`：发布前验证 npm 包边界、本地 tarball 安装、生成项目 skeleton 和 Flask 启动。
- `bin/check-docs`：检查实现前文档包，确保需求基线事实分类、PC 后台形态、技术方案、workflow 契约和测试用例完整。
- `bin/check-demo`：检查 README、运行入口、本地 URL、方案选择、技能记录、工作流说明、mock fallback，并可在运行中应用上探测 primary loop 页面和业务 API JSON envelope。
...
刚生成的目录是待 Agent 填充的工作区，不是开箱即过的完成品。此时 Flask 模板可以启动，但 `docs/requirements.md`、`docs/solution-options.md`、`docs/tech-plan.md`、`docs/workflow-integration.md`、`docs/test-cases.md`、`docs/skill-trace.md`、`docs/test-report.md` 仍是占位骨架；只有 Agent 完成需求、方案选择、技术方案、workflow 文档、技能记录、测试报告和 README 更新后，generated app 才应该通过 `./bin/check-demo .`。
...
- 验收脚本 `bin/check-demo` 和文档质量脚本 `bin/check-docs`。
- 项目 `README.md` 骨架。
- 标准产物骨架 `docs/requirements.md`、`docs/solution-options.md`、`docs/tech-plan.md`、`docs/workflow-integration.md`、`docs/test-cases.md`、`docs/skill-trace.md`、`docs/test-report.md`。讲稿、展示稿和评委追问材料不属于 `check-demo` 的标准产物。
...
`bin/check-docs` 只检查实现前文档包：`docs/requirements.md`、`docs/tech-plan.md`、`docs/workflow-integration.md` 和 `docs/test-cases.md`。它不启动应用，不探测页面，也不替代实现后的 `check-demo`。

`bin/check-demo` 有两个模式：

- `./bin/check-demo --skeleton .`：检查 fresh generated project 的结构、禁止的源拷贝和可运行的 shell。
- `./bin/check-demo .`：检查 Agent 完成后的完整应用：需求文档、方案选择、技术方案、workflow 集成、技能记录、测试报告、README，以及可选的真实页面/API 探针。
...
bin/check-docs              实现前文档包质量检查
docs/                       参考资料、检查清单和 workflow 文档
docs/reference/week-report/ 周报系统原始参考材料（仅维护用，不进入生成项目）
docs/sample-doc-validation/ 样例业务文档包验证材料（社区报修、合同风险审核、设备巡检派单）
...
templates/docs/             生成项目的标准产物骨架（requirements.md、solution-options.md、tech-plan.md、workflow-integration.md、test-cases.md、skill-trace.md、test-report.md）
```
...
1. 先执行 `npm pack --dry-run` 和本地安装包验证，确认发布包边界、生成项目结构和 `./bin/check-demo --skeleton .`。
2. 再发布 npm 包，并在另一台设备上执行全局安装验证。
3. 运行 `demo-ai-app <project-name>`，确认生成项目和模板启动命令在干净环境可用；`./bin/check-demo .` 只应在 Agent 填完标准产物后通过。
4. 根据跨设备测试结果收敛包体范围、README 和默认技能链。

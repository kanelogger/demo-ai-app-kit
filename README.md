...
- `templates/docs/requirements/solution-options.md`：要求在技术方案和实现前输出 3 个可选方案、推荐方案和用户/Agent 选择记录。
- `templates/docs/execution/skill-trace.md`：记录实际使用的 skills、产物、跳过原因和命令证据。
...
- `templates/docs/`：生成项目的标准文档骨架，按 `requirements/`、`technical/`、`execution/` 分组。
...
- 增加 `bin/demo-ai-app`：支持 `demo-ai-app <project-name>` 生成待 Agent 填充的项目工作区，自动复制 `AGENTS.md`、`kit/` 初始化资料、`bin/check-demo`、模板代码和标准产物骨架。
...
刚生成的目录是待 Agent 填充的工作区，不是开箱即过的完成品。此时 Flask 模板可以启动，但 `docs/requirements/requirements.md`、`docs/requirements/solution-options.md`、`docs/technical/tech-plan.md`、`docs/technical/workflow-integration.md`、`docs/technical/test-cases.md`、`docs/execution/skill-trace.md`、`docs/execution/test-report.md` 仍是占位骨架；只有 Agent 完成需求、方案选择、技术方案、workflow 文档、技能记录、测试报告和 README 更新后，generated app 才应该通过 `./bin/check-demo .`。

生成目录包含：

- 基于 `templates/flask-admin-shell/` 的可运行 Flask 骨架。
- `kit/template-patterns/` 模式库供实现阶段参考。
- 项目级 `AGENTS.md` 和 `kit/prompts/opencode-entry.md`。
- `kit/skills/` 核心技能目录（默认产品质量链路所需）。
- 验收脚本 `bin/check-demo` 和文档质量脚本 `kit/checkers/check-docs`。
- 项目 `README.md` 骨架。
- 标准产物骨架按 `docs/requirements/`、`docs/technical/`、`docs/execution/` 分组。讲稿、展示稿和评委追问材料不属于 `check-demo` 的标准产物。

Agent 完成后的验收：

```bash
./kit/checkers/check-docs .
./bin/check-demo .
```

`kit/checkers/check-docs` 只检查实现前文档包：`docs/requirements/requirements.md`、`docs/technical/tech-plan.md`、`docs/technical/workflow-integration.md` 和 `docs/technical/test-cases.md`。它不启动应用，不探测页面，也不替代实现后的 `check-demo`。
...
templates/docs/             生成项目的标准产物骨架（requirements/、technical/、execution/）
```

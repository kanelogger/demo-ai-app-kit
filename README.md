...
- 增加 `bin/demo-ai-app`：支持 `demo-ai-app <project-name>` 生成待 Agent 填充的项目工作区，自动复制 `AGENTS.md`、入口 prompt、核心技能、`bin/check-demo`、模板代码和标准产物骨架。
...
刚生成的目录是待 Agent 填充的工作区，不是开箱即过的完成品。此时 Flask 模板可以启动，但 `docs/requirements.md`、`docs/solution-options.md`、`docs/tech-plan.md`、`docs/workflow-integration.md`、`docs/skill-trace.md`、`docs/test-report.md` 仍是占位骨架；只有 Agent 完成需求、方案选择、技术方案、workflow 文档、技能记录、测试报告和 README 更新后，generated app 才应该通过 `./bin/check-demo .`。

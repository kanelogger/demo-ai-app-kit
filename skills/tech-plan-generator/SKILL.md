...
- Optional skill routing must stay small: use `api-and-interface-design` for complex contracts, `security-and-hardening` for auth/user input/external calls, `webapp-testing` for browser verification, `debugging-and-error-recovery` for failing builds/tests, `code-review-and-quality` for final risk review, `architecture-diagram` for architecture visuals, and `guizang-ppt-skill` only when a web PPT is explicitly needed.

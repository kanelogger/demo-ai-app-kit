...
- `agent-browser` installed. Check:
  ...
  If not installed, stop and report that `agent-browser` is missing. Do not continue without it.
...
For this project, do not depend on the removed `ce-test-browser` skill. Use Phase 3 below for port detection and dev-server startup, and prefer the retained `webapp-testing` skill for normal generated-app browser verification.
...
Determine the port (priority: explicit `--port` > `AGENTS.md`/`CLAUDE.md` > `package.json` dev script > `.env*` `PORT=` > default `3000`). If a server is already listening, reuse it; otherwise start the project's dev command in the background and wait for the port to come up.
